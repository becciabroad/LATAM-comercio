...
From another data base in tradestatistics, I can choose only countries belonging to the "Americas"
...
I rename the column "reporter_iso" to be able to merge it with the second data frame from the API
```{r comercio rename, message=FALSE}
comercio_mundo_modificado <- rename(comercio_mundo_api, country_iso = 'reporter_iso')
```

Merging the two data frames to have the information about the Americas:
```{r comercio LATAM merge, message=FALSE}
comercio_latam_reducido <- merge(x = comercio_mundo_modificado, y = countries_LATAM,
      by = "country_iso") %>%
  drop_na() #limpio los N/A, this excludes a lot of observations/countries - I need to check this again

data.frame(comercio_latam_reducido)
```
